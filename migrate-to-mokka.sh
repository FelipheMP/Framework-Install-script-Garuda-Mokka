#!/usr/bin/env bash
# migrate-to-mokka.sh
# Thoroughly reviewed Garuda → Mokka migration
...
if pacman -Qs garuda-dr460nized > /dev/null 2>&1; then
    ...
    if [[ "$confirm_dragon" =~ ^[Yy]$ ]]; then
        ...
        sudo pacman -Rs --noconfirm garuda-dr460nized \
          && echo "Removed 'garuda-dr460nized'." \
          || _err "Warning: Failed to remove 'garuda-dr460nized'."
    else
...
if [[ ! "$confirm_update" =~ ^[Nn]$ ]]; then
    ...
    sudo pacman -Syu --noconfirm \
      && echo "System updated." \
      || _err "Warning: System update encountered errors."
else
...
if [[ ! "$confirm_install" =~ ^[Nn]$ ]]; then
    ...
    sudo pacman -S --noconfirm garuda-mokka firedragon-catppuccin \
      && echo "Installed 'garuda-mokka' and 'firedragon-catppuccin'." \
      || _err "Warning: Failed to install Mokka or Firedragon."
else
...
if [[ -f /etc/skel/.config/Kvantum/Mokka.kvconfig ]]; then
    ...
    cp /etc/skel/.config/Kvantum/Mokka.kvconfig ~/.config/Kvantum/ \
      && echo "Copied Kvantum 'Mokka.kvconfig' to ~/.config/Kvantum/." \
      || _err "Warning: Failed to copy Kvantum config."
else
...
if [[ ! "$confirm_starship" =~ ^[Nn]$ ]]; then
    if [[ -f /etc/skel/.config/starship-mokka.toml ]]; then
        mkdir -p ~/.config
        cp /etc/skel/.config/starship-mokka.toml ~/.config/starship.toml \
          && echo "Starship config copied to ~/.config/starship.toml." \
          || _err "Warning: Failed to copy Starship config."
    else
...
if [[ ! "$confirm_fastfetch" =~ ^[Nn]$ ]]; then
    ...
    if [[ -f /etc/skel/.config/fastfetch/mokka.jsonc ]]; then
        ...
        cp /etc/skel/.config/fastfetch/mokka.jsonc ~/.config/fastfetch/ \
          && echo "  Copied 'mokka.jsonc' to '~/.config/fastfetch/'."
    else
    ...
    if [[ -f ~/.config/fish/config.fish ]] && grep -qE 'fastfetch' ~/.config/fish/config.fish; then
        ...
        sed -i 's#fastfetch.*#fastfetch --load-config ~/.config/fastfetch/mokka.jsonc#' ~/.config/fish/config.fish \
          && echo "  Updated Fish config to launch Fastfetch with 'mokka.jsonc'."
    fi
    if [[ -f ~/.bashrc ]] && grep -qE 'fastfetch' ~/.bashrc; then
        sed -i 's#fastfetch.*#fastfetch --load-config ~/.config/fastfetch/mokka.jsonc#' ~/.bashrc \
          && echo "  Updated Bash config to launch Fastfetch with 'mokka.jsonc'."
    fi
else
...
if [[ ! "$confirm_bat" =~ ^[Nn]$ ]]; then
    ...
    if [[ -f /etc/skel/.config/bat/config ]]; then
        mkdir -p ~/.config/bat
        cp /etc/skel/.config/bat/config ~/.config/bat/ \
          && echo "  Copied bat config to '~/.config/bat/'."
    else
    ...
    if [[ -d /etc/skel/.config/bat/themes ]]; then
        ...
        cp -r /etc/skel/.config/bat/themes/* ~/.config/bat/themes/ \
          && echo "  Copied bat themes to '~/.config/bat/themes/'."
    else
...
if [[ ! "$confirm_lock" =~ ^[Nn]$ ]]; then
    if [[ -f /etc/skel/.config/kscreenlockerrc ]]; then
        ...
        cp /etc/skel/.config/kscreenlockerrc ~/.config/kscreenlockerrc \
          && echo "Copied 'kscreenlockerrc' for Mokka screen locker."
    else
...
if [[ ! "$confirm_splash" =~ ^[Nn]$ ]]; then
    ...
    echo "Skipping Garuda splash setup."
fi
...
if [[ ! "$confirm_konsole" =~ ^[Nn]$ ]]; then
    ...
    if [[ -f /usr/share/konsole/Mokka.profile ]]; then
        ...
        cp /usr/share/konsole/Mokka.profile ~/.local/share/konsole/Mokka.profile \
          && echo "  Copied '/usr/share/konsole/Mokka.profile' to '~/.local/share/konsole/'."
...
if [[ "$confirm_games" =~ ^[Yy]$ ]]; then
    sudo pacman -S --noconfirm garuda-rani-games garuda-games-meta \
      ...
      || _err "Warning: Failed to install gaming meta-packages."
else
